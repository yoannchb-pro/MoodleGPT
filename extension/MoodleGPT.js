!(function (e) {
  ...
  const l =
    ...
    c = { url: null, system: { role: s.SYSTEM, content: l }, history: [] };
  function a(e) {
    const n = [],
    ...
    const r = o.reduce((e, n) => e + n) + 3 * n[0].length + 1,
      i = '\n' + Array(r).fill('-').join('') + '\n',
      s = n.map(e => '| ' + e.map((e, n) => e.padEnd(o[n], ' ')).join(' | ') + ' |');
    return s.shift() + i + s.join('\n');
  }
  function u(n, t) {
  ...
  function f(e, n, t) {
    const o = n[0];
    if (1 !== n.length || 'true' !== o.getAttribute('contenteditable')) return !1;
    if (e.typing) {
      let e = 0;
      o.addEventListener('keydown', function (n) {
        if (('Backspace' === n.key && (e = t.response.length + 1), e > t.response.length)) return;
        n.preventDefault(), (o.textContent = t.response.slice(0, ++e)), o.focus();
        const r = document.createRange();
        r.selectNodeContents(o), r.collapse(!1);
        const i = window.getSelection();
        null !== i && (i.removeAllRanges(), i.addRange(r));
      });
  ...
  function d(e, n, t) {
    var o, r;
    const i = n[0];
    if (1 !== n.length || 'number' !== i.type) return !1;
    const s =
      null ===
        (r =
    ...
    if (e.typing) {
      ...
      i.addEventListener('keydown', function (n) {
        n.preventDefault(),
          'Backspace' === n.key && (e = s.length + 1),
          e > s.length || ('.' === s.slice(e, e + 1) && ++e, (i.value = s.slice(0, ++e)));
      });
    } else i.value = s;
    return !0;
  }
  function m(e, n, t) {
    const s = null == n ? void 0 : n[0];
    if (!s || 'radio' !== s.type) return !1;
    ...
    return (
      e.mouseover
        ? a.addEventListener('mouseover', () => (a.checked = !0), { once: !0 })
        : (a.checked = !0),
      !0
    );
  }
  function p(e, n, t) {
    ...
    for (const n of l) {
      const t = o(n, c);
      e.logs && t.value && r.bestAnswer(t.value, t.similarity);
      const i = t.element;
      e.mouseover
        ? i.addEventListener('mouseover', () => (i.checked = !0), { once: !0 })
        : (i.checked = !0);
    }
    return !0;
  }
  function h(e, n, t) {
    ...
    const s = t.normalizedResponse.split('\n');
    e.logs && r.array(s);
    for (let t = 0; t < n.length && s[t]; ++t) {
      const l = n[t].querySelectorAll('option'),
        c = Array.from(l)
          .map(e => {
            ...
            return {
              element: e,
              value: i(null !== (n = e.textContent) && void 0 !== n ? n : '')
            };
          })
          .filter(e => '' !== e.value),
        a = o(s[t], c);
      ...
      null !== f &&
        (e.mouseover
          ? f.addEventListener('click', () => (u.selected = !0), { once: !0 })
          : (u.selected = !0));
    }
    return !0;
  }
  function g(e, n, t) {
    ...
    if (e.typing) {
      ...
      o.addEventListener('keydown', function (n) {
        n.preventDefault(),
          'Backspace' === n.key && (e = t.response.length + 1),
          e > t.response.length || (o.value = t.response.slice(0, ++e));
      });
  ...
  function v(e) {
    return n(this, void 0, void 0, function* () {
      ...
      const t = (function (e) {
          ...
          const o = e.querySelectorAll('.qtext table');
          for (const e of o) n = n.replace(e.innerText, '\n' + a(e) + '\n');
          return i(n, !1);
        })(e.form),
        o = e.form.querySelectorAll(e.inputQuery),
        l = yield (function (e, t) {
          return n(this, void 0, void 0, function* () {
            const n = location.hostname + location.pathname;
            (e.history && c.url === n) || ((c.url = n), (c.history = []));
            const o = new AbortController(),
              r = setTimeout(() => o.abort(), 15e3),
              l = { role: s.USER, content: t },
              a = yield fetch('https://api.openai.com/v1/chat/completions', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  Authorization: `Bearer ${e.apiKey}`
                },
                signal: e.timeout ? o.signal : null,
                body: JSON.stringify({
                  model: e.model,
                  messages: [c.system, ...c.history, l],
                  temperature: 0.8,
                  top_p: 1,
                  presence_penalty: 1,
                  stop: null
            ...
            clearTimeout(r);
            const u = (yield a.json()).choices[0].message.content;
            return (
              e.history && (c.history.push(l), c.history.push({ role: s.ASSISTANT, content: u })),
              { question: t, response: u, normalizedResponse: i(u) }
            );
          });
        })(e.config, t).catch(e => ({ error: e })),
        v = 'object' == typeof l && 'error' in l;
      if (
        (e.config.cursor &&
          (e.questionElement.style.cursor = e.config.infinite || v ? 'pointer' : 'initial'),
        v)
      )
        console.error(l.error);
      else
        switch ((e.config.logs && (r.question(t), r.response(l)), e.config.mode)) {
          case 'clipboard':
            !(function (e) {
              e.config.infinite || e.removeListener(), u(e.config, e.gptAnswer);
            })({
              config: e.config,
          ...
          case 'question-to-answer':
            !(function (e) {
              const n = e.questionElement;
              e.removeListener();
              const t = n.textContent;
              (n.textContent = e.gptAnswer.response),
                (n.style.whiteSpace = 'pre-wrap'),
                n.addEventListener('click', function () {
                  (n.style.whiteSpace = ''), (n.textContent = t);
                });
            })({
              gptAnswer: l,
              questionElement: e.questionElement,
  ...
  const y = [],
    w = [];
  function A(e) {
    const n = w.findIndex(n => n.element === e);
    if (-1 !== n) {
      const e = w.splice(n, 1)[0];
  ...
  function E(n) {
    if (w.length > 0) {
      for (const e of w)
        n.cursor && (e.element.style.cursor = 'initial'),
          e.element.removeEventListener('click', e.fn);
      return n.title && e('Removed'), void (w.length = 0);
    }
    const t =
    ...
    for (const e of o) {
      ...
      w.push({ element: o, fn: r }), o.addEventListener('click', r);
    }
    n.title && e('Injected');
  }
    ...
    n.code
      ? (function (e) {
          document.body.addEventListener('keydown', function (n) {
            y.push(n.key),
              y.length > e.code.length && y.shift(),
              y.join('') === e.code && ((y.length = 0), E(e));
          });
        })(n)
      : E(n);
  });
